...
export function start_stream(worker: Worker, opts: Omit<ServerToWorkerStreamMessage_Add_Stream, 'type'>) {
    ...
    try {
        ...
        logger.error(error, `Failed to start stream ${opts.id}. Worker may have crashed.`);
    }
}
...
export function stop_stream(worker: Worker, opts: {
    ...
    try {
        ...
        logger.error(error, `Failed to stop stream ${opts.id}. Worker may have crashed.`);
    }
}
...
export function set_moment_state(worker: Worker, opts: {
    ...
    try {
        ...
        logger.error(error, `Failed to set moment state for ${opts.media_id}. Worker may have crashed.`);
    }
}
